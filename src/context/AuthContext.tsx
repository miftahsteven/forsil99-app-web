...
interface AuthContextType {
  ...
  isLoading: boolean;
  login: (identifier: string, pass: string, recaptchaToken?: string, coords?: { latitude?: number; longitude?: number }) => Promise<any>;
  logout: () => void;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (
    ...
    const res = await loginWithEmailOrPhone(identifier, pass, recaptchaToken, coords);
    if (res.user) {
      setUser(res.user);
    }
    if (res.profile) {
